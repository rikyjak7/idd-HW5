...
def mapper(mediated_schema_columns): 
    data = DataframeExtractor.extract_dataframe_by_extension("Files/output_globaldata.json")
    mediated_df = pd.DataFrame(columns=mediated_schema_columns)    

    mediated_df["Company Name"] = data["name"]
    mediated_df["Headquarters Country"] = data["headquarters"]
    ...
    mediated_df["Official Website"] = data["website"]
    mediated_df["Market Valuation"] = data["market_cap"]
    mediated_df["Phone Number"] = data["telephone"]
