...
def mapper(mediated_schema_columns): 
    data = DataframeExtractor.extract_dataframe_by_extension("Files/company_social_urls.csv")
    mediated_df = pd.DataFrame(columns=mediated_schema_columns)    

    mediated_df["Company Name"] = data["Company"]
    mediated_df["Social Media - Facebook"] = data["Facebook"]
    mediated_df["Social Media - Twitter"] = data["Twitter"]
    mediated_df["Social Media - Instagram"] = data["Instagram"]
    mediated_df["Social Media - Pinterest"] = data["Pinterest"]
